...
( function FBXLoader() {
    ...
    THREE.FBXLoader.prototype = {
        ...
        parse: function ( allNodes ) {

            function parseGeometries ( node ) {

                // has not geo, return []
                if ( !( 'Geometry' in node.subNodes ) ) { return []; }

                // has many
                var geoCount = 0;
                for ( var geo in node.subNodes.Geometry ) {
                ...
                if ( geoCount > 0 ){
                    ...
                    for ( geo in node.subNodes.Geometry ) {
                        if ( node.subNodes.Geometry[geo].attrType === 'Mesh' ) {
                            res.push( parseGeometry( node.subNodes.Geometry[geo] ) );
                        }
                    }

                } else {

                    res.push( parseGeometry( node.subNodes.Geometry ) );
            ...
            function parseGeometry ( node ) {
                ...
                geo.addBones( hierarchy.hierarchy );
                ...
                // TODO: for debug
                var url = 'obj/Char_UV_Texture.gif';
                ...
                geometry.bones = geo.bones;  // hierarchy.hi;
                geometry.skinIndices = weights.skinIndices;
                geometry.skinWeights = weights.skinWeights;

                addAnimation( geometry, weights.matrices, animations );
                ...
                var mesh = null;
                //if ( geo.bones === undefined || geo.skins === undefined || geo.uvs < 10 ){
                if ( geo.bones === undefined || geo.skins === undefined || geometry.animations === undefined || geometry.animations.length < 1 ){
            ...
            function addAnimation ( geometry, matrices, animations ) {

                var animationdata = { "name": 'animationtest', "fps": 30, "length": animations.length, "hierarchy": [] };

                for (var i=0; i < geometry.bones.length; ++i) {
                    var name = geometry.bones[i].name;
                    name = name.replace( /.*:/, '' );
                    animationdata.hierarchy.push( { parent: geometry.bones[i].parent, name: name, keys:[] } );
                }
                ...
                var genKey = function ( animNode, bone ) {
                    ...
                    // TODO: if values[ frame ] not exists
                    try {
                ...
                var clip = THREE.AnimationClip.parseAnimation( animationdata, geometry.bones );
            ...
            function parseMaterial ( node ) {
                ...
                matName  = node.attrName;

                eff = new Effect();
                console.log( eff.parse( node ) );
            ...
            hierarchy  = ( new Bones() ).parseHierarchy( allNodes );
            weights    = ( new Weights() ).parse( allNodes, hierarchy );
            animations = ( new Animation() ).parse( allNodes, hierarchy );
            console.timeEnd( 'FBXLoader: ObjectParser' );

            console.time( 'FBXLoader: GeometryParser' );
            geometries = parseGeometries( allNodes.Objects );
            // materials  = parseMaterials( allNodes.Objects );
